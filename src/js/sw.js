...
chrome.runtime.onMessageExternal.addListener((msg, sender, sendResponse) => {
  ...
  switch (msg.request) {
    ...
    case "decrypt":
      const array = objectToUInt8array(msg.content); // Messaging messes up types, here I'm converting it back to the correct type: Uint8array
      ...
      decrypt(readableStream, writableStream, msg.usk, msg.identity).then(
        ...
        () => {
          console.error("Error in decrypting email");
          sendResponse(
    ...
    case "hidden policies":
      const uint8array = objectToUInt8array(msg.content); // Messaging messes up types, here I'm converting it back to the correct type: Uint8array
      getHiddenPolicies(uint8array)
        ...
        .catch((err) => {
          console.error("Error in getting hidden policies: ", err);
        });
  }
  return true;
});
